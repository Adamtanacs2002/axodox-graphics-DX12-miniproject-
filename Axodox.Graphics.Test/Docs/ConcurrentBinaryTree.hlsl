 ...
 *
 */
cbt_Node cbt_DecodeNode(uint nodeID /*leafID*/)
{
    cbt_Node node = cbt_CreateNode(1u, 0);                  // HeapID = 1

    while (cbt_HeapRead(node) > 1u) {                       // CBT[heapID] > 1
        cbt_Node leftChild = cbt_LeftChildNode_Fast(node);
        uint cmp = cbt_HeapRead(leftChild);                 // CBT[heapID]
        uint b = nodeID < cmp ? 0u : 1u;                    // 2k+0 OR 2k+1

        node = leftChild;                                   // 2k
        node.id |= b;                                       // +0 OR +1
        nodeID -= cmp * b;                                  // leafID = leafID - CBT[heapID]
    }
